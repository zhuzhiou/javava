...
public class ShelfInfoController {
    ...
    @PostMapping(value = "")
    public ShelfInfo create(int boxNum, @RequestParam(value = "remark", required = false) String remark, String name) {
    ...
    public void maintain(@PathVariable Long shelfId,@RequestParam String operateType) {
        if(ShelfConstants.OPERATE_SHELF_ONLINE.equals(operateType)
                || ShelfConstants.OPERATE_SHELF_OFFLINE.equals(operateType)){
            shelfInfoService.update(shelfId, null, operateType);
        }
    }
